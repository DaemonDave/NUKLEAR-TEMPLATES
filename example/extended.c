...
// added for combo box type and show...
#include "common.h"

const char* algorithm[NUM_ALGORITHMS];
const char* algorithmDescriptions[NUM_ALGORITHMS];
void set_algorithm()
{
        algorithm[OIT_SIMPLE] =
            "OIT_SIMPLE\0";
        algorithm[OIT_LINKEDLIST] =
            "OIT_LINKEDLIST\0";
        algorithm[OIT_LOOP] =
            "OIT_LOOP\0";
        algorithm[OIT_LOOP64] =
            "OIT_LOOP64\0";
        algorithm[OIT_INTERLOCK] =
            "OIT_INTERLOCK\0";
        algorithm[OIT_SPINLOCK] =
            "OIT_SPINLOCK\0";
        algorithm[OIT_WEIGHTED] =   
            "OIT_WEIGHTED\0";
}
void set_algorithmtext()
{
        algorithmDescriptions[OIT_SIMPLE] =
            "A simple A-buffer method. Each pixel or sample stores the first OIT_LAYERS fragments it processes, and tail-blends the rest. It then sorts these fragments by depth and blends the result onto the opaque objects.\0";
        algorithmDescriptions[OIT_LINKEDLIST] =
            ...
            "fragments by depth, and tail-blends the rest.\0";
        algorithmDescriptions[OIT_LOOP] =
            ...
            "The compositing shader then blends the sorted fragments together.\0";
        ...
        algorithmDescriptions[OIT_INTERLOCK] =
            ...
            "compositing shader then blends the sorted fragments together.\0";
        algorithmDescriptions[OIT_SPINLOCK] =
            ...
            "then blends the sorted fragments together.\0";
...
//! \note : Must include various sections like NK_INCLUDE_FIXED_TYPES to be added which will get rid of implicit declarations (missing functions from linker)
//! \note : DRE2024
/** linker failure example
 * extended.c:(.text+0x3a7f3): undefined reference to `nk_labelf'
/usr/local/bin/ld: extended.c:(.text+0x3a890): undefined reference to `nk_labelf'
/usr/local/bin/ld: extended.c:(.text+0x3c6a1): undefined reference to `nk_labelf'
/usr/local/bin/ld: extended.c:(.text+0x3c7a7): undefined reference to `nk_tooltipf'
/usr/local/bin/ld: extended.c:(.text+0x3c811): undefined reference to `nk_labelf'
/usr/local/bin/ld: extended.c:(.text+0x3d6c8): undefined reference to `nk_tooltipf'
/usr/local/bin/ld: extended.c:(.text+0x3e8f7): undefined reference to `nk_labelf'
...
//! \note included to maintain headers and scope
#include "overview.c"
...
//    static const char *items[] = {"Item 0","item 1","item 2"};
    ...
    static int check = 1;
/// \def used in this section to understand variables interplay
#define WINDWWIDTH 800
#define WINDWHEIGHT 600
    int i;
    nk_style_set_font(ctx, &media->font_20->handle);
    // struct nk_rect {float x,y,w,h;};
    if (nk_begin(ctx, "Grid Demo", nk_rect(420, 20, WINDWWIDTH, WINDWHEIGHT), NK_WINDOW_TITLE|NK_WINDOW_BORDER|NK_WINDOW_MOVABLE|NK_WINDOW_NO_SCROLLBAR))
    {
		// set font style
        ...
        // label for the combobox in right aligned
        nk_label(ctx, "algorithm: ", NK_TEXT_LEFT);
        // expanded from 200 to 400 height
        // struct nk_vec2 {float x,y;};
        if (nk_combo_begin_label(ctx, algorithm[selected_item], nk_vec2(nk_widget_width(ctx), 400))) 
        {
			// nk_layout_row_dynamic(struct nk_context *ctx, float height, int cols)
			// 20 is twice the height of a single line 11 is good for font 18
            nk_layout_row_dynamic(ctx, 11, 1);
            for (i = 0; i < NUM_ALGORITHMS; ++i)
                if (nk_combo_item_label(ctx, algorithm[i], NK_TEXT_LEFT))
                    selected_item = i; // selects from index
			//! \note added tooltip for similar function - FAILED MAKES ILLEGAL POPUP
            //if (i != -1)
            //    nk_tooltipf(ctx, "%s", algorithmDescriptions[i] );			
            //! END tooltip insertion
            nk_combo_end(ctx);
        }
        // NK_API void nk_label_colored_wrap(struct nk_context *ctx, const char *str, struct nk_color color)
        // struct nk_color {nk_byte r,g,b,a;};
        // added variable wrapped length and colour
        struct nk_color out;
        out.r = 7;
        out.g = 6;
        out.b = 4;
        out.a = 7; 
        // special adjustable if selection was made
        if (selected_item != -1)
        {
			// NK_API void nk_layout_row_static(struct nk_context *ctx, float height, int item_width, int cols)
			// create a static label that takes whole screen
			nk_layout_row_static(ctx, 200, WINDWWIDTH-20, 1);
			// NK_API void nk_label_wrap(struct nk_context *ctx, const char *str)
			nk_label_wrap(ctx, algorithmDescriptions[selected_item] );
		}
    }
    nk_end(ctx);
